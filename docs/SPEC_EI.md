...
```
              Fact   Intent   Intent(c)   Hint   Insight   EI(启发)   EI(审计)
Manager        W       W         W         -        R         W          -
Proposer       R       -         W         R        -         -          -
Worker_P       R       R         -         R        W         -          -
Worker_N       R       R         -         R        W         -          -
...
- EI(启发) = Manager 记录的启发式评估（辅助选择）
- EI(审计) = Auditor 的 result_EI + 四维审计

### 特殊规则

1. **Fact/Hint 降格/删除**：只有 HumanGate 通过 `/interrupt` + `Fact-` / `Hint-` 可以降格或标记无效（不是真正删除，而是状态标记）
2. **Fact/Hint 升格**：
   - 路径①：Worker → Auditor 提取 fact_candidates → Manager 裁决 → 写入 Facts
   - 路径②：Hint → Auditor 标注 suggest_promote → Manager 裁决 → 写入 Facts
3. **Auditor**：只输出信号（fact_candidates, hint_candidates, valley_report），不直接写 Facts/Hints
